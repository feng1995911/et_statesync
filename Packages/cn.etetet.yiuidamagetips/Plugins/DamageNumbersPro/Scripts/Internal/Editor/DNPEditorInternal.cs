...
namespace DamageNumbersPro.Internal
{
    public static class DNPEditorInternal
    {
        ...
        public static void PrepareInspector(DamageNumberEditor damageNumberEditor)
        {
            ...
            allPresets.Add("风格", Resources.LoadAll<DNPPreset>("DNP/Style"));
            allPresets.Add("淡入", Resources.LoadAll<DNPPreset>("DNP/Fade In"));
            allPresets.Add("淡出", Resources.LoadAll<DNPPreset>("DNP/Fade Out"));
            allPresets.Add("行为", Resources.LoadAll<DNPPreset>("DNP/Behaviour"));
        ...
        static void DrawTabs(Rect lastRect)
        {
            ...
            currentTab = GUI.Toolbar(lastRect, currentTab, new string[] { "主页", "文本", "淡入", "淡出" }, buttonStyle);
            lastRect.height -= 1;
            BoxRect(lastRect);

            string rotAndScaleText = "旋转/大小";
            if (currentWidth < 388)
            {
                rotAndScaleText = "<size=11>旋转/大小</size>";

                if (currentWidth < 356)
                {
                    rotAndScaleText = "<size=10>旋转/大小</size>";

                    if (currentWidth < 324)
                    {
                        rotAndScaleText = "<size=9>旋转/大小</size>";

                        if (currentWidth < 303)
                        {
                            rotAndScaleText = "<size=8>旋转/大小</size>";

                            if (currentWidth < 276)
                            {
                                rotAndScaleText = "<size=7>旋转/大小</size>";
                            }
                        }
                    }
                }
            ...
            string spamText = "分组控制";
            if (currentWidth < 340)
            {
                spamText = "<size=11>分组控制</size>";

                if (currentWidth < 324)
                {
                    spamText = "<size=10>分组控制</size>";

                    if (currentWidth < 304)
                    {
                        spamText = "<size=9>分组控制</size>";

                        if (currentWidth < 367)
                        {
                            spamText = "<size=8>分组控制</size>";
                        }
                    }
                }
            }

            string performanceText = "表演";
            if (currentWidth < 336)
            {
                performanceText = "<size=11>表演</size>";

                if (currentWidth < 310)
                {
                    performanceText = "<size=10>表演</size>";

                    if (currentWidth < 292)
                    {
                        performanceText = "<size=9>表演</size>";
                    }
                }
            }

            string movementText = "运动";
            if (currentWidth < 293)
            {
                movementText = "<size=11>运动</size>";

                if (currentWidth < 282)
                {
                    movementText = "<size=10>运动</size>";

                    if (currentWidth < 263)
                    {
                        movementText = "<size=9>运动</size>";
                    }
                }
            }
        ...
        public static void Externalnspectors(bool isMesh)
        {
            ...
            //Tab Names:
            string textMeshProTab = "文本网格";
            string materialTab = "材质";
            if (damageNumbers != null && damageNumbers.Length > 1)
            {
                textMeshProTab = damageNumbers.Length + " 文本网格";
                materialTab = "材质";
            }
            ...
            //Draw Tab:
            currentEditor = GUI.Toolbar(tabRect, currentEditor, new string[] { "预设", materialTab, textMeshProTab });
            ...
            else if (currentEditor == 2)
            {
                ...
                if (damageNumbers.Length > 1)
                {
                    ...
                    Label("花式检查器对<b>多个</b>损坏数不起作用。");
                    Label("您也可以<b>手动选择</b> text-mesh-pro组件。");
                    Label("-抱歉给您带来不便。");
        ...
        {
            PresetCategory("风格", isMesh);
            EditorGUILayout.Space(12);
            PresetCategory("淡入", isMesh);
            EditorGUILayout.Space(12);
            PresetCategory("淡出", isMesh);
            EditorGUILayout.Space(12);
            PresetCategory("行为", isMesh);
        }
        ...
        public static bool CheckStructure(DamageNumberEditor damageNumberEditor)
        {
            ...
            if (isStructureFlawed)
            {
                ...
                if (isOutdated)
                {
                    EditorGUILayout.LabelField("4.0版本改变了伤害数的结构。", labelStyle);
                    EditorGUILayout.LabelField("点击上面的按钮<b>升级</b>此损坏数。", labelStyle);
                }
                else
                {
                    EditorGUILayout.LabelField("缺少重要的组件。", labelStyle);
                    EditorGUILayout.LabelField("点击上面的按钮<b>准备</b>这个伤害数。", labelStyle);
                }
        ...
        public static void PrepareMeshStructure(GameObject go)
        {
            ...
            //Undo:
            Undo.RegisterCreatedObjectUndo(go, "创建新的伤害数(网格)。");
        }
