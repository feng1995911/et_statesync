...
namespace I2.Loc
{
    public partial class LocalizationEditor
    {
        ...
        const int Script_Tool_MaxVariableLength = 50;
        ...
        {
            EditorApplication.update -= BuildScriptWithSelectedTerms;
            BuildScriptWithSelected();
        }

        public static void BuildScriptWithSelected()
        {
            var lastLanguage = LocalizationManager.CurrentLanguage;

            //默认切换到中文导出,增加了多语言的注释显示多语言内容 这里的语言就是注释的结果
            var codeLanguage = YIUIConstHelper.Const.I2CodeCommentLanguage;
            LocalizationManager.CurrentLanguage = string.IsNullOrEmpty(codeLanguage) ? "Chinese" : codeLanguage;
        ...
        static void BuildScriptWithTrans(StringBuilder sbTrans)
        {
            List<string> Categories = LocalizationManager.GetCategories();
            foreach (string Category in Categories)
            {
        ...
        static void BuildScriptWithTerms(StringBuilder sbTerms)
        {
            List<string> Categories = LocalizationManager.GetCategories();
            foreach (string Category in Categories)
            {
        ...
        static List<string> ScriptTool_GetSelectedTermsInCategory(string Category)
        {
        ...
        static void BuildScriptCategoryTrans(StringBuilder sbTrans, string Category, List<string> AdjustedTerms, List<string> Terms)
        {
            if (Category == LanguageSourceData.EmptyCategory)
            {
                for (int i = 0; i < Terms.Count; ++i)
                {
                    ...
                    var key = Terms[i];
                    var content = LocalizationManager.GetTranslation(key).Replace('\n', ' ');
                    sbTrans.AppendLine($"		/// <summary>");
                    sbTrans.AppendLine($"		/// {content}");
                    sbTrans.AppendLine($"		/// </summary>");
        ...
        static void BuildScriptCategoryTerms(StringBuilder sbTerms, string Category, List<string> AdjustedTerms, List<string> Terms)
        {
            if (Category == LanguageSourceData.EmptyCategory)
            {
                for (int i = 0; i < Terms.Count; ++i)
                {
                    ...
                    var key = Terms[i];
                    var content = LocalizationManager.GetTranslation(key).Replace('\n', ' ');
                    sbTerms.AppendLine($"		/// <summary>");
                    sbTerms.AppendLine($"		/// {content}");
                    sbTerms.AppendLine($"		/// </summary>");
                    sbTerms.AppendLine($"		public const string {AdjustedTerms[i]} = \"{key}\";");
                }
            }
            else
                for (int i = 0; i < Terms.Count; ++i)
        ...
        static string ScriptTool_AdjustTerm(string Term, bool allowFullLength = false)
        {
        ...
        static void ScriptTool_EnumerateDuplicatedTerms(List<string> AdjustedTerms)
        {
            string lastTerm = "$";
            int Counter = 1;
        ...
        static bool IsCSharpKeyword(string variableName)
        {
