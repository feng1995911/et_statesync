...
namespace I2.Loc
{
    ...
    public class Localize : MonoBehaviour
    {
        ...
        [NonSerialized]
        public Dictionary<string, Object> mAssetDictionary = new Dictionary<string, Object>(StringComparer.Ordinal); //This is used to overcome the issue with Unity not serializing Dictionaries
        ...
        [SerializeField]
        public EventCallback LocalizeCallBack = new EventCallback(); //LocalizeCallBack is deprecated. Please use LocalizeEvent instead.
        ...
        public void OnLocalize(bool Force = false)
        {
            ...
            {
                Debug.LogError($"[{FinalTerm}],没有找到多语言,请检查预制体设置,{this.gameObject.name}", this.gameObject);
            }
            #endif

            SecondaryTranslation = "";
            if (!YIUIConstHelper.Const.I2CloseSecondaryTranslation)
            {
                ...
                {
                    Debug.LogError($"[{FinalTerm}],[{FinalSecondaryTerm}],没有找到多语言,请检查预制体设置,{this.gameObject.name}", this.gameObject);
                }
                #endif
            }
