...
namespace Fullscreen.LogicBlock.Runtime
{
    ...
    [Image(typeof(IconStatusEffect), ColorTheme.Type.TextLight)]
    [Description("A status effect reference from a Block")]
    [Serializable]
    [HideLabelsInEditor]
    public class BlockGetStatusEffectReference : PropertyTypeGetStatusEffect
    {
        [SerializeField] private PropertyGetBlockListReference m_ListReference = new PropertyGetBlockListReference();

        public override StatusEffect Get(Args args)
        {
            ...
            {
                Debug.LogWarning("Invalid Block reference parameters for status effect.");
                return null;
            }
            ...
            return reference is StatusEffect statusEffect ? statusEffect : null;
        }

        public override StatusEffect Get(GameObject gameObject)
        {
            return Get(new Args(gameObject));
        }

        public BlockGetStatusEffectReference() : base() { }

        public BlockGetStatusEffectReference(Block block, string instructionListId, string referenceName) : this()
        {
        ...
        public static PropertyGetStatusEffect Create() => new PropertyGetStatusEffect(new BlockGetStatusEffectReference());

        public static PropertyGetStatusEffect Create(Block block, string instructionListId, string referenceName)
            => new PropertyGetStatusEffect(new BlockGetStatusEffectReference(block, instructionListId, referenceName));
