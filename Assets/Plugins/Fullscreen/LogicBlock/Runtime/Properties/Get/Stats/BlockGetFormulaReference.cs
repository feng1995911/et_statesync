...
namespace Fullscreen.LogicBlock.Runtime
{
    ...
    [Image(typeof(IconFormula), ColorTheme.Type.TextLight)]
    [Description("A formula reference from a Block")]
    [Serializable]
    [HideLabelsInEditor]
    public class BlockGetFormulaReference : PropertyTypeGetFormula
    {
        [SerializeField] private PropertyGetBlockListReference m_ListReference = new PropertyGetBlockListReference();

        public override Formula Get(Args args)
        {
            ...
            {
                Debug.LogWarning("Invalid Block reference parameters for formula.");
                return null;
            }
            ...
            return reference is Formula formula ? formula : null;
        }

        public override Formula Get(GameObject gameObject)
        {
            return Get(new Args(gameObject));
        }

        public BlockGetFormulaReference() : base() { }

        public BlockGetFormulaReference(Block block, string instructionListId, string referenceName) : this()
        {
        ...
        public static PropertyGetFormula Create() => new PropertyGetFormula(new BlockGetFormulaReference());

        public static PropertyGetFormula Create(Block block, string instructionListId, string referenceName)
            => new PropertyGetFormula(new BlockGetFormulaReference(block, instructionListId, referenceName));
