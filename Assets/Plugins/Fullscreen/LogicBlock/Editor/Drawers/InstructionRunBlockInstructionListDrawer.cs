...
{
    [CustomPropertyDrawer(typeof(InstructionRunBlockInstructionList))]
    public class InstructionRunBlockInstructionListDrawer : PropertyDrawer
    {
        ...
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            PropertyField selectedListField = new PropertyField(selectedListProp, "Instruction List");
        ...
        {
            if (blockProp?.objectReferenceValue != null && blockProp.objectReferenceValue is UnityEngine.Object obj && obj != null)
            {
                blockSerializedObject = new SerializedObject(blockProp.objectReferenceValue);
                blockSerializedObject.Update();
            }
            else
            {
    ...
    private int GetReferencesCount(SerializedProperty blockProp, string instructionId)
    {
        ...
        blockSerializedObject.Update();
        SerializedProperty actionListsProp = blockSerializedObject.FindProperty("m_ActionLists");
        if (actionListsProp == null || !actionListsProp.isArray)
        {
            return 0;
        }

        for (int i = 0; i < actionListsProp.arraySize; i++)
        {
            SerializedProperty listProp = actionListsProp.GetArrayElementAtIndex(i);
        ...
        {
            if (property?.serializedObject == null || property.serializedObject.targetObject == null) return;
