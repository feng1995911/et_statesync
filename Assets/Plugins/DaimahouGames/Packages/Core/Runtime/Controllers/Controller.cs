...
namespace DaimahouGames.Runtime.Core
{
    ...
    public class Controller : ScriptableObject
    {
        ...
        public void Possess(Pawn pawn)
        {
            ...
            if (m_Pawn != null && m_Pawn.Get<Character>() != null)
            {
                m_Pawn.Get<Character>().Player.IsControllable = false;
            }
            
            if (pawn.Get<Character>() != null && !pawn.Get<Character>().IsPlayer)
            {
                Debug.LogWarning("Possessed pawn is not a player character, controls will not be fully enabled - if you need the character to become controllable via GC, please set it as player character", pawn);
            }
