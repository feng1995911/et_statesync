...
namespace GameCreator.Editor.Perception
{
    public class PerceptionView : VisualElement
    {
        ...
        private void RefreshAwareness()
        {
            ...
            IReadOnlyDictionary<int, Tracker> awarenessMap = this.m_Perception.TrackerList;
            
            for (int i = this.m_AwarenessViews.Count; i < awarenessMap.Count; ++i)
            {
                AwarenessView awarenessView = new AwarenessView();
                this.m_AwarenessViews.Add(awarenessView);
            }

            this.m_AwarenessTitle.style.display = awarenessMap.Count > 0
                ? DisplayStyle.Flex
                : DisplayStyle.None;

            int indexView = 0;
            foreach (KeyValuePair<int,Tracker> entry in awarenessMap)
            {
                Tracker tracker = entry.Value;
                if (tracker.Target == null) continue;
                
                AwarenessView awarenessView = this.m_AwarenessViews[indexView];
