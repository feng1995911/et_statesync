...
namespace GameCreator.Runtime.Perception
{
    ...
    public class Perception : MonoBehaviour, ISpatialHash
    {
        ...
        public IReadOnlyDictionary<int, Tracker> TrackerList => this.m_Cortex.TrackerList;
        ...
        public void RelayAwareness(Perception other)
        {
            ...
            foreach (KeyValuePair<int, Tracker> entry in this.m_Cortex.TrackerList)
            {
                if (entry.Value.Target == null) continue;
                Tracker otherTracker = other.GetTracker(entry.Value.Target);
                
                if (otherTracker == null) continue;
                if (otherTracker.Awareness > entry.Value.Awareness) continue;
                
                otherTracker.Awareness = entry.Value.Awareness;
                newAwareness = true;
            }
